...
for m=-38:1:38
    for n=-38:1:38
        if mod(n,2)==1
            uin_loop=circ(sqrt(((X-m*lens_pitch)-1/2*lens_pitch).^2+(Y-n*lens_pitch).^2)/w); % Incident field
            Iin_loop=(abs(uin_loop)).^2; % Incident intensity
            uout_mla_loop=focus(uin_loop,L,lambda,f,m*lens_pitch+1/2*lens_pitch,n*lens_pitch); % Field after microlens array 
            ...
            %%% Analytical        
            Ipattern_loop_ana=(w^2/lz)^2.*(jinc(w/lz*sqrt(((X-m*lens_pitch)-1/2*lens_pitch).^2+(Y-n*lens_pitch).^2))).^2;
            Ipattern_ana=Ipattern_ana+Ipattern_loop_ana;

        else 
            uin_loop=circ(sqrt((X-m*lens_pitch).^2+(Y-n*lens_pitch).^2)/w); % Incident field
            Iin_loop=(abs(uin_loop)).^2; % Incident intensity
            uout_mla_loop=focus(uin_loop,L,lambda,f,m*lens_pitch,n*lens_pitch); % Field after microlens array 
            upattern_loop=propTF(uout_mla_loop,L,lambda,z); % Field of illumination pattern at MLA focal plane
            Ipattern_loop=(abs(upattern_loop)).^2; % Intensity of illumination pattern at MLA focal plane
            Iin=Iin+Iin_loop;
            Ipattern=Ipattern+Ipattern_loop;
            %%% Analytical        
            Ipattern_loop_ana=(w^2/lz)^2.*(jinc(w/lz*sqrt((X-m*lens_pitch).^2+(Y-n*lens_pitch).^2))).^2;
            Ipattern_ana=Ipattern_ana+Ipattern_loop_ana;
        end
